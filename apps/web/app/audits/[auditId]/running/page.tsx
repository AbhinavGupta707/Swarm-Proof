...
import { auditMetrics, auditSuccessRate, auditTimeline, auditTimeToValue } from "@/lib/audit-presenters";
...
export default async function RunningAuditPage({ params }: { params: Promise<{ auditId: string }> }) {
  ...
  const timeline = auditTimeline(audit);
  const eventCount = eventData?.events.length ?? audit.eventCount ?? 0;
  const metrics = auditMetrics({ ...audit, eventCount });
          ...
          {audit.runs.map((run) => {
            ...
            return (
              <article key={run.id} className="rounded-ui border border-line bg-panel p-5 shadow-sm">
                    ...
                    <div className="flex h-full flex-col justify-between">
                      <div className="flex items-center justify-between gap-2 text-xs text-slate-500">
                        <span className="inline-flex items-center gap-1">
                          ...
                          {run.viewport ?? (run.mode === "mobile" ? "390 x 844" : "desktop viewport")}
                        </span>
                ...
                <Link className="mt-4 inline-flex min-h-11 items-center rounded-ui border border-line px-3 py-2 text-sm font-semibold hover:bg-mist" href={`/audits/${audit.id}/replay/${run.id}`}>
                  View replay
                </Link>
              </article>
            );
        ...
        <section className="mt-6 grid gap-6 lg:grid-cols-[1.2fr_0.8fr]">
          ...
          <aside className="rounded-ui border border-line bg-ink p-5 text-white shadow-sm">
            ...
            <p className="mt-3 text-sm leading-6 text-slate-300">
              The report, generated test, share link, and Novus proof are ready to inspect for this run.
            </p>
