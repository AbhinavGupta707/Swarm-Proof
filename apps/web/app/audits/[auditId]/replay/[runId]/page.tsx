...
export default async function ReplayPage({ params }: { params: Promise<{ auditId: string; runId: string }> }) {
  ...
  const run = demoAudit.runs.find((item) => item.id === runId) ?? demoAudit.runs[0];
  const frames = demoAudit.evidenceFrames.filter((frame) => frame.runId === run.id);
    ...
    <main className="section surface-grid">
      <div className="page-shell">
        <div className="flex flex-wrap items-end justify-between gap-4">
          <div>
            <p className="font-mono text-sm font-semibold text-indigo">Replay {run.id}</p>
            <h1 className="mt-2 text-4xl font-semibold tracking-normal">{run.persona} evidence timeline</h1>
            <p className="mt-3 max-w-3xl leading-7 text-slate-700">{run.summary}</p>
          </div>
        ...
        <section className="mt-6 rounded-ui border border-line bg-panel p-5">
          ...
          <ol className="mt-4 grid gap-3">
            {demoAudit.steps.filter((step) => step.runId === run.id).map((step) => (
